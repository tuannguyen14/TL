...
def onLogin(email, number):
    ...
    resp = asyncio.run(onPost(url, data, headersLogin))
    if resp['accessToken'] != None:
        global accessToken
        print(resp)
        accessToken.append(resp['accessToken'])
...
def onStartGame(currentNumber):
    ...
    try:
        resp = asyncio.run(onPost(url, data))
        global gameData
        gameData.append(resp)
        onPrintCurrentAccount()
        #print(result)
